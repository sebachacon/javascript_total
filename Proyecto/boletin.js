...
let notas = [80, 55, 98, 93, 60] // notas para las funciones
...
function listarNotas() { 
    ...
    for (let nota of notas) { // recorre el array
        let lista = document.createElement("li") // se crea los elemento li en el ul para escribir las notas
...
function hayAplazados() { 
    ...
    for (let nota of notas) { // Recorre todas las notas
        if (nota < 40) { // Si LA nota es menor o igual a 40 hay una aplazada
            aplazo.textContent = "Sí" //Escribe en pantalla que hay aplazados
            return // Retorna en caso de que si haya aplazados
        } else {
            aplazo.textContent = "No" // Si la nota mo es igual o menor a 40 se sigue mostrando NO
        }
    }
}